(ns specljs.reporting
  (:require [clojure.string :as string :refer [split join]]
            [goog.string] ;cljs bug?
            [specljs.config :refer [*reporters* *color?* *full-stack-trace?*]]
            [specljs.platform :refer [endl file-separator failure-source stack-trace cause print-stack-trace elide-level?]]
...
(defmethod report-run specljs.results.PassResult [result reporters]
  (doseq [reporter reporters]
    (report-pass reporter result)))
(defmethod report-run specljs.results.FailResult [result reporters]
  (doseq [reporter reporters]
    (report-fail reporter result)))
(defmethod report-run specljs.results.PendingResult [result reporters]
  (doseq [reporter reporters]
    (report-pending reporter result)))
(defmethod report-run specljs.results.ErrorResult [result reporters]
  (doseq [reporter reporters]
